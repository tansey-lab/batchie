...
workflow NEXT_BATCH_PLATE {
    ...
    def excludes = params.excludes == null ? [] : params.excludes.toString().tokenize(',')

    // channel: [ val(meta), path(screen), path(thetas), path(distance_matrix_chunks), val(n_chunks), val(reveal), val(excludes) ]
