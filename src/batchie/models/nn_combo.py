...
def train_model(model, X_tensor, y_tensor):
    ...
    print(
        f"Training Size: {train_size}, Validation size: {val_size}, Testing Size: {test_size}"
    )
    PRINT_EVERY = 100
...
def plot_training_loss_validation(train_losses, val_losses, model):
    ...
    plt.savefig(
        f"tmp_results/BATCHIERUNNING_training_validation_curve_{model}.pdf",
        bbox_inches="tight",
    )
    plt.savefig(
        f"tmp_results/BATCHIERUNNING_training_validation_curve_{model}.png",
        bbox_inches="tight",
        dpi=500,
    )
    plt.close()
...
class NN_DrugComboTheta(Theta):
    ...
    def predict_viability(self, data: ScreenBase) -> np.ndarray:
        if data.treatment_arity != 2:
            raise ValueError(
                "NNDrugCombo only supports data sets with combinations of 2 treatments"
            )
        ...
        with torch.no_grad():
            for inputs, targets in test_dataloader:
                outputs = self.model(
                    inputs
                ).squeeze()  # Squeeze the output tensor to remove any extra dimensions
                predicted_viability_values.extend(outputs.numpy())
        self.predicted_values = np.clip(
            predicted_viability_values, a_min=0.01, a_max=0.99
        )
...
class NN_DrugCombo(BayesianModel):
    ...
    def get_model_state(self) -> NN_DrugComboTheta:
        return NN_DrugComboTheta(
            model=self.model, X_tensor=self.X_tensor, y_tensor=self.y_tensor
        )
    ...
    def _add_observations(self, data: ScreenBase):
        ...
        obs_tensor = torch.tensor(
            data.observations, dtype=torch.float32, device=self.device
        )
        ...
        X_tensor = torch.stack(
            [cls_tensor, dd1s_tensor, dd2s_tensor, td1s_tensor, td2s_tensor], dim=1
        )
