...
class BatchIterator:
    ...
    def __iter__(self):
        ...
        conc_combo_idxs = np.random.choice(int((self.n_grid)**2), size=self.n, replace=True)
        self.conc_combo_idxs = torch.from_numpy(conc_combo_idxs).long().reshape(self.n,2)
        return self
...
class ConcentrationGrid:
    def __init__(self, 
        ...
        if isinstance(log_conc_range, (list, tuple)) and (len(log_conc_range)==2):
            ...
            min_conc = {drugname2idx[drugname]:lower for drugname, (lower, upper) in log_conc_range.items()}
            max_conc = {drugname2idx[drugname]:upper for drugname, (lower, upper) in log_conc_range.items()}
        else:
            raise ValueError("log_conc_range must be dict or pair. drugname2idx must be dict. keys of log_conc_range/drugname2idx must match.")
        ...
        self.conc_grid = torch.from_numpy(np.concatenate([min_v[:, np.newaxis], conc_grid, max_v[:, np.newaxis]], axis=1)).float()
