...
class GridComboResults(ThetaHolder):
    def __init__(
        ...
        self.sample_embeddings = torch.zeros(n_thetas, self.n_unique_samples, 1, self.n_embedding_dimensions)
        self.single_drug_embeddings = torch.zeros(n_thetas, self.n_unique_drugs, self.n_grid-1, self.n_embedding_dimensions)
        self.combo_drug_embeddings = torch.zeros(n_thetas, self.n_unique_drugs, self.n_grid-1, self.n_embedding_dimensions)

        self.sample_sigma_embeddings = torch.zeros(n_thetas, self.n_unique_samples, 1, self.n_sigma_embedding_dimensions)
        self.single_drug_sigma_embeddings = torch.zeros(n_thetas, self.n_unique_drugs, self.n_grid-1, self.n_sigma_embedding_dimensions)
        self.combo_drug_sigma_embeddings = torch.zeros(n_thetas, self.n_unique_drugs, self.n_grid-1, self.n_sigma_embedding_dimensions)
    ...
    def combine(self, other):
        ...
        if self.n_unique_drugs != other.n_unique_drugs:
            raise ValueError(
                "Cannot combine with different number of unique drugs"
            )
    ...
    def _save_theta(
        self, sample: GridComboSample, variance: float, sample_index: int
    ):
        ...
        self.single_drug_sigma_embeddings[sample_index] = sample.single_drug_sigma_embeddings
        self.combo_drug_sigma_embeddings[sample_index] = sample.combo_drug_sigma_embeddings
    ...
    def save_h5(self, fn: str):
        with h5py.File(fn, "w") as f:
            f.create_dataset("sample_embeddings", data=self.sample_embeddings, compression="gzip")
            f.create_dataset("single_drug_embeddings", data=self.single_drug_embeddings, compression="gzip")
            f.create_dataset("combo_drug_embeddings", data=self.combo_drug_embeddings, compression="gzip")
            f.create_dataset("single_drug_sigma_embeddings", data=self.single_drug_sigma_embeddings, compression="gzip")
            f.create_dataset("combo_drug_sigma_embeddings", data=self.combo_drug_sigma_embeddings, compression="gzip")
            f.create_dataset("sample_sigma_embeddings", data=self.sample_sigma_embeddings, compression="gzip")
            f.create_dataset("mean_obs_sigma", data=self.mean_obs_sigma, compression="gzip")
...
class ComboGridFactorModel(BayesianModel):
    def __init__(self, 
        ...
        self.n_grid = n_grid
        self.conc_grid = ConcentrationGrid(log_conc_range, self.drugname2idx, n_grid, log_conc_padding=log_conc_padding)
    ...
    def predict_grid(self, sample_embeddings:torch.FloatTensor, drug_embeddings:torch.FloatTensor):
        N, *_ = sample_embeddings.shape
        
        interval_grid = torch.softmax((sample_embeddings * drug_embeddings).sum(dim=-1), dim=-1) ## N x n_grid-1
        cum_sum = torch.cumsum(interval_grid, dim=1)

        ## Prediction over entire grid
        pred_grid = torch.concatenate( ((interval_grid - cum_sum + 1.0), torch.zeros(N,1)), dim=1 ) ## N x n_grid
    ...
    def predict_single(self, 
        ...
        single_effect = lin_p*single_pred_grid.gather(1, ub_idx).squeeze() + (1. - lin_p)*single_pred_grid.gather(1, (ub_idx-1)).squeeze()
    ...
    def predict_synergy(self, 
        ...
        synergy_effect = (lin_p_1*lin_p_2*u1u2) + (lin_p_1*(1.-lin_p_2)*u1l2) + ((1. - lin_p_1)*lin_p_2*l1u2) + ((1.-lin_p_1)*(1.-lin_p_2)*l1l2)
    ...
    def mean_combo(self, 
            ...
            single_effect_1 = self.predict_single(sample_embeddings=sample_embeddings, 
                                                  single_drug_embeddings=single_drug_embeddings_1, 
                                                  ub_idx=ub_idx_1, 
                                                  lin_p=lin_p_1)
            
            single_effect_2 = self.predict_single(sample_embeddings=sample_embeddings, 
                                                  single_drug_embeddings=single_drug_embeddings_2, 
                                                  ub_idx=ub_idx_2, 
                                                  lin_p=lin_p_2)
            

            synergy_effect = self.predict_synergy(sample_embeddings=sample_embeddings, 
    ...
    def scale_combo(self, 
            ...
            single_sigma_raw = torch.sum(sample_sigma_embeddings * single_drug_sigma_embeddings_1, dim=-1)
    ...
    def smoothing_combo_prediction(self, 
                                   sample_embeddings:torch.FloatTensor, 
        ...
        conc_dist = torch.abs(concs11 - concs21)/conc_scale1 + torch.abs(concs12 - concs22)/conc_scale2
    ...
    def model(self, batch:tuple, n_data:int):
        ...
        obs = None
        if len(batch) == 9:
            sample_ids, drug_ids_1, drug_ids_2, ub_idx_1, ub_idx_2, lin_p_1, lin_p_2, obs, conc_combo_idxs = batch
        elif len(batch) == 8:
            sample_ids, drug_ids_1, drug_ids_2, ub_idx_1, ub_idx_2, lin_p_1, lin_p_2, obs = batch
        elif len(batch) == 7:
            sample_ids, drug_ids_1, drug_ids_2, ub_idx_1, ub_idx_2, lin_p_1, lin_p_2 = batch
        ...
        sample_embed = pyro.sample("sample_embed", dist.Normal(torch.zeros(self.n_samples, 1, self.n_dims), sample_embed_sigma).to_event(3))
        
        single_drug_embed_sigma = pyro.sample('single_drug_embed_sigma', dist.Gamma(1.0, 1.0))
        single_drug_embed = pyro.sample("single_drug_embed", dist.Normal(torch.zeros(self.n_drugs, self.n_grid-1, self.n_dims), single_drug_embed_sigma).to_event(3))
        
        
        combo_drug_embed_sigma = pyro.sample('combo_drug_embed_sigma', dist.Gamma(1.0, 1.0))
        combo_drug_embed = pyro.sample("combo_drug_embed", dist.Normal(torch.zeros(self.n_drugs, self.n_grid, self.n_dims), combo_drug_embed_sigma).to_event(3))
        ...
        with pyro.plate('viability_plate', size=n_data, subsample_size=batch_size):
            ...
            lin1 = lin_p_1
            lin2 = lin_p_2
            mu = self.mean_combo(sample_embeddings=samp, 
            ...
            sigma = self.scale_combo(sample_sigma_embeddings=sample_sigma_embed[sample_ids], 
            ...
            if (self.smooth_scale>0.0) and (conc_combo_idxs is not None):
                synergy_diff, conc_dist = self.smoothing_combo_prediction(sample_embeddings=samp, 
                ...
                diff = pyro.sample('synergy_diff', dist.Laplace(synergy_diff, conc_dist), obs=zeros)
        return(mu)
    ...
    def add_observations(self, data: ScreenBase):
        ...
        self.sample_ids = np.concatenate([self.sample_ids, data.sample_ids[mask]])

        with np.errstate(divide='ignore'):
            self.log_conc1 = np.concatenate([self.log_concs_1, np.log10(data.treatment_doses[mask,0])])
            self.log_conc2 = np.concatenate([self.log_concs_2, np.log10(data.treatment_doses[mask,1])])

        drugname2idx = self.drugname2idx.copy()
        drugname2idx[data.control_treatment_name] = -1
        drug_ids_1 = np.array([drugname2idx[drugname] for drugname in data.treatment_names[mask,0]])
        drug_ids_2 = np.array([drugname2idx[drugname] for drugname in data.treatment_names[mask,1]])
        ...
        self.drug_ids_1[self.log_conc1==-np.inf] = -1
        self.drug_ids_2[self.log_conc2==-np.inf] = -1

        self.y = np.concatenate([self.y, np.clip(data.observations[mask], a_min=0.0, a_max=1.0)])
    ...
    def fit(self):
        ...
        mask = self.drug_ids_1 < 0
        self.drug_ids_1[mask] = self.drug_ids_2[mask].copy()
        self.drug_ids_2[mask] = -1
        self.log_conc1[mask] = self.log_conc2[mask].copy()
        ...
        ub_idx_1, lin_p_1 = self.conc_grid.lookup_conc(log_concs=log_conc1, drug_ids=self.drug_ids_1)
        ub_idx_2, lin_p_2 = self.conc_grid.lookup_conc(log_concs=log_conc2, drug_ids=self.drug_ids_2)
        ...
        batch_iterator = BatchIterator(sample_ids, 
                                       drug_ids_1, 
                                       drug_ids_2, 
                                       ub_idx_1, 
                                       ub_idx_2, 
